...
module.exports = {
    ...
    detectMatches: async({_image,_faces,draw}) => {
        const image = await canvas.loadImage(_image);
        const labeledDescriptors = await Promise.all(_faces.map(async face=>{
            const faceImage = await canvas.loadImage(face.image);
            const faceImageFd = await faceapi.detectSingleFace(faceImage).withFaceLandmarks().withFaceDescriptor()
            return new faceapi.LabeledFaceDescriptors(face.name,[faceImageFd.descriptor])
        }))
